...
class NeuralNetwork:
    ...
    def train(self,
        ...
        if validation_data is None:
            validation_data_size = x.shape[0] // 10
    ...
    def generate_batches(self, x: np.array, y: np.array) -> iter:
        """
    ...
    def decide(self, x: np.array) -> np.array:
        """
    ...
    def save(self, path: str) -> None:
        ...
        while path.exists():
            path = path.with_stem(f"{path}_{count}")
            count += 1
